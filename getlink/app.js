...
let adminCookiePoolItems = [];
let adminCookiePoolConfig = null;
let adminCookiePoolColumnFilters = {};
let adminCookiePoolLoaded = false;
let adminCookiePoolLoading = false;
...
const ADMIN_POOL_SYSTEM_COLUMNS = [
    { key: 'cookie', label: 'Cookie', group: 'primary', editable: true },
    { key: 'status', label: 'Status', group: 'check' },
    { key: 'reason', label: 'Reason', group: 'check' },
    { key: 'plan', label: 'Plan', group: 'check' },
    { key: 'planNew', label: 'Plan new', group: 'check-new' },
    { key: 'country', label: 'Country', group: 'check' },
    { key: 'paymentHold', label: 'Payment hold', group: 'check' },
    { key: 'profiles', label: 'Profiles', group: 'check' },
    { key: 'maxStreams', label: 'Max streams', group: 'check' },
    { key: 'maxStreamsNew', label: 'Max streams new', group: 'check-new' },
    { key: 'videoQuality', label: 'Video quality', group: 'check' },
    { key: 'overloadOutcome', label: 'Overload', group: 'check' },
    { key: 'overloadSignal', label: 'Signal', group: 'check' },
    { key: 'lastCheckedAt', label: 'Last check', group: 'check' },
    { key: 'checkCount', label: 'Check count', group: 'check' },
    { key: 'mark', label: 'Mark', group: 'manage', editable: true },
    { key: 'note', label: 'Note', group: 'manage', editable: true },
    { key: 'usedCount', label: 'Used', group: 'manage' },
    { key: 'lastUsedAt', label: 'Last used', group: 'manage' },
    { key: 'lastUsedSlot', label: 'Last slot', group: 'manage' },
    { key: 'lastUsedScope', label: 'Last source', group: 'manage' },
    { key: 'actions', label: 'Action', group: 'action' }
];
const ADMIN_POOL_SYSTEM_COLUMN_MAP = ADMIN_POOL_SYSTEM_COLUMNS.reduce((map, column) => {
    map[column.key] = column;
    return map;
}, {});
const ADMIN_POOL_DEFAULT_COLUMN_ORDER = ADMIN_POOL_SYSTEM_COLUMNS.map((column) => column.key);
...
function setAdminCookiePoolState(text, mode = 'idle') {
    const node = el('adminCookiePoolState');
    if (!node) return;
    node.textContent = String(text || '').trim();
    setStateClass(node, mode);
}
...
function parseCookiePoolPaste(text = '') {
    const lines = String(text || '').split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
    if (lines.length === 0) return [];
    const headerCells = lines[0].split('\t').map((cell) => cell.trim().toLowerCase());
    const headerMap = {
        cookie: ['cookie', 'cookie raw', 'cookie_raw', 'netflix cookie'],
        mark: ['mark', 'usage', 'used'],
        note: ['note', 'ghi chu', 'ghi chú'],
        status: ['status', 'trang thai', 'trạng thái'],
        reason: ['reason', 'ly do', 'lý do'],
        plan: ['plan'],
        planNew: ['plan new', 'goi cuoc moi', 'gói cước mới'],
        country: ['country', 'quoc gia', 'quốc gia'],
        paymentHold: ['payment hold', 'payment_hold', 'on_payment_hold'],
        profiles: ['profiles'],
        maxStreams: ['max streams', 'max_streams'],
        maxStreamsNew: ['max streams new', 'so thiet bi moi', 'số thiết bị mới'],
        videoQuality: ['video quality', 'video_quality'],
        overloadOutcome: ['overload outcome', 'overload_outcome'],
        overloadSignal: ['overload signal', 'overload_signal']
    };
    const hasKnownHeader = headerCells.some((cell) => Object.values(headerMap).some((names) => names.includes(cell)));
    const firstCellLooksLikeCookie = /netflixid|secure.netflix|;/i.test(headerCells[0] || '');
    const hasHeader = hasKnownHeader || (lines.length > 1 && !firstCellLooksLikeCookie);
    const keyByIndex = {};
    const labelByIndex = {};
    if (hasHeader) {
        headerCells.forEach((cell, index) => {
            let matched = false;
            Object.entries(headerMap).forEach(([key, names]) => {
            ...
            if (!matched && cell) {
                keyByIndex[index] = `custom:${normalizeAdminPoolCustomKey(cell)}`;
                labelByIndex[index] = lines[0].split('\t')[index].trim();
            }
        });
    }
    ...
    return lines.slice(hasHeader ? 1 : 0).map((line) => {
        ...
        cells.forEach((cell, index) => {
            ...
            if (key.startsWith('custom:')) {
                ...
                row.customLabels = row.customLabels && typeof row.customLabels === 'object' ? row.customLabels : {};
                const customKey = key.slice('custom:'.length);
                row.customFields[customKey] = cell;
                row.customLabels[customKey] = labelByIndex[index] || customKey;
                return;
            }
            row[key] = cell;
        });
        if (!row.cookie) row.cookie = cells[0] || '';
        return row;
    }).filter((row) => String(row.cookie || '').trim());
}

function normalizeAdminPoolCustomKey(value = '') {
    return String(value || '')
        .trim()
        .toLowerCase()
        .normalize('NFD')
        ...
        .replace(/[^a-z0-9_]+/g, '_')
        .replace(/^_+|_+$/g, '')
        .slice(0, 40) || `col_${Date.now().toString(36)}`;
}

function normalizeAdminCookiePoolConfig(input = {}) {
    const source = input && typeof input === 'object' ? input : {};
    const seen = new Set();
    const customColumns = (Array.isArray(source.customColumns) ? source.customColumns : []).map((column) => {
        const key = normalizeAdminPoolCustomKey(column && column.key);
        const label = String(column && column.label ? column.label : key).trim() || key;
        return { key, label };
    }).filter((column) => {
    ...
    const allowed = new Set([...ADMIN_POOL_DEFAULT_COLUMN_ORDER, ...customColumns.map((column) => `custom:${column.key}`)]);
    const columnOrder = [];
    (Array.isArray(source.columnOrder) ? source.columnOrder : ADMIN_POOL_DEFAULT_COLUMN_ORDER).forEach((key) => {
        const normalized = String(key || '').trim();
        if (allowed.has(normalized) && !columnOrder.includes(normalized)) columnOrder.push(normalized);
    });
    ADMIN_POOL_DEFAULT_COLUMN_ORDER.forEach((key) => {
    ...
    const hiddenColumns = Array.from(new Set(
        (Array.isArray(source.hiddenColumns) ? source.hiddenColumns : [])
            ...
            .filter((key) => allowed.has(key) && key !== 'actions')
    ));
    const selectionRules = {
        groups: (Array.isArray(source.selectionRules && source.selectionRules.groups)
            ? source.selectionRules.groups
            : []).map((group) => ({
            conditions: (Array.isArray(group && group.conditions) ? group.conditions : []).map((condition) => ({
                ...
                operator: String(condition && condition.operator ? condition.operator : 'equals').trim(),
                value: String(condition && condition.value !== undefined && condition.value !== null ? condition.value : '').trim()
            })).filter((condition) => condition.field)
        })).filter((group) => group.conditions.length > 0)
    };
    return { customColumns, columnOrder, hiddenColumns, selectionRules };
}

function getAdminCookiePoolConfig() {
    adminCookiePoolConfig = normalizeAdminCookiePoolConfig(adminCookiePoolConfig || {});
    return adminCookiePoolConfig;
}

function getAdminPoolColumns(options = {}) {
    const config = getAdminCookiePoolConfig();
    const customMap = new Map(config.customColumns.map((column) => [`custom:${column.key}`, column]));
    const columns = config.columnOrder.map((key) => {
        if (ADMIN_POOL_SYSTEM_COLUMN_MAP[key]) return ADMIN_POOL_SYSTEM_COLUMN_MAP[key];
        const custom = customMap.get(key);
        if (custom) return { key, customKey: custom.key, label: custom.label, group: 'custom', editable: true };
        return null;
    }).filter(Boolean);
    if (options && options.includeHidden) return columns;
    const hidden = new Set(config.hiddenColumns || []);
    return columns.filter((column) => column.key === 'actions' || !hidden.has(column.key));
}

function getAdminPoolRawValue(item = {}, column = {}) {
    if (column.group === 'custom') {
        return item.customFields && typeof item.customFields === 'object'
            ? item.customFields[column.customKey]
            : '';
    }
    return item[column.key];
}

function getAdminPoolFieldValue(item = {}, field = '') {
...
function matchesAdminPoolCondition(item = {}, condition = {}) {
    const actual = String(getAdminPoolFieldValue(item, condition.field) ?? '').trim().toLowerCase();
...
function matchesAdminPoolSelectionRules(item = {}) {
    const rules = getAdminCookiePoolConfig().selectionRules || { groups: [] };
    if (String(item.status || '').trim().toLowerCase() !== 'pass') return false;
    if (!Array.isArray(rules.groups) || rules.groups.length === 0) return true;
    return (Array.isArray(rules.groups) ? rules.groups : []).every((group) => (
        Array.isArray(group && group.conditions)
            && group.conditions.length > 0
            && group.conditions.some((condition) => matchesAdminPoolCondition(item, condition))
    ));
}

function getFilteredCookiePoolItems() {
    const filter = String(el('adminCookiePoolFilter')?.value || 'all').trim();
    const source = Array.isArray(adminCookiePoolItems) ? adminCookiePoolItems : [];
    const columns = getAdminPoolColumns({ includeHidden: true });
    return source.filter((item) => {
        if (filter === 'unused' && Number(item && item.usedCount || 0) > 0) return false;
        if (filter === 'used' && Number(item && item.usedCount || 0) <= 0) return false;
        if (filter === 'payment_hold' && String(item && item.reason || '').trim() !== 'payment_hold') return false;
        if (filter === 'unknown_plan' && String(item && item.reason || '').trim() !== 'unknown_plan') return false;
        if (filter === 'duplicate') {
            const reason = String(item && item.reason || '').trim();
            const mark = String(item && item.mark || '').trim();
            if (reason !== 'duplicate' && mark !== 'duplicate') return false;
        }
        if (['pass', 'die', 'new'].includes(filter) && String(item && item.status || '').trim() !== filter) return false;
        return columns.every((column) => {
            const query = String(adminCookiePoolColumnFilters[column.key] || '').trim().toLowerCase();
            if (!query || column.key === 'actions') return true;
            const value = getAdminPoolRawValue(item, column);
            return String(value === undefined || value === null ? '' : value).toLowerCase().includes(query);
        });
    });
}

function appendPoolCell(row, text = '', className = '') {
...
function createAdminPoolInput(itemId = '', key = '', value = '', className = '') {
    const input = document.createElement('input');
    input.className = `text-input admin-pool-input${className ? ` ${className}` : ''}`;
    input.type = 'text';
    input.value = String(value || '');
    input.dataset.poolItemId = itemId;
    input.dataset.poolField = key;
    return input;
}

function getAdminPoolCellValue(item = {}, column = {}) {
    if (column.group === 'custom') {
        return item.customFields && typeof item.customFields === 'object' ? item.customFields[column.customKey] : '';
    }
    const value = item[column.key];
    if (column.key === 'lastCheckedAt' || column.key === 'lastUsedAt') return value ? formatDateTime(value) : '';
    return value;
}

function renderAdminCookiePoolHeader(columns = []) {
    const head = el('adminCookiePoolTableHead');
    if (!head) return;
    head.innerHTML = '';
    const row = document.createElement('tr');
    columns.forEach((column, index) => {
        const th = document.createElement('th');
        th.className = `admin-pool-col-${column.group || 'system'}`;
        const title = document.createElement('div');
        title.className = 'admin-pool-th-title';
        const label = document.createElement('span');
        label.textContent = column.label;
        title.appendChild(label);
        if (column.group === 'custom') {
            const renameBtn = document.createElement('button');
            renameBtn.className = 'admin-pool-mini-btn';
            renameBtn.type = 'button';
            renameBtn.textContent = 'Sua';
            renameBtn.addEventListener('click', () => adminRenameCookiePoolColumn(column.customKey));
            const deleteBtn = document.createElement('button');
            deleteBtn.className = 'admin-pool-mini-btn';
            deleteBtn.type = 'button';
            deleteBtn.textContent = 'Xoa';
            deleteBtn.addEventListener('click', () => adminDeleteCookiePoolColumn(column.customKey));
            title.appendChild(renameBtn);
            title.appendChild(deleteBtn);
        }
        const moves = document.createElement('div');
        moves.className = 'admin-pool-col-moves';
        const leftBtn = document.createElement('button');
        leftBtn.className = 'admin-pool-mini-btn';
        leftBtn.type = 'button';
        leftBtn.textContent = '<';
        leftBtn.disabled = index === 0;
        leftBtn.addEventListener('click', () => adminMoveCookiePoolColumn(column.key, -1));
        const rightBtn = document.createElement('button');
        rightBtn.className = 'admin-pool-mini-btn';
        rightBtn.type = 'button';
        rightBtn.textContent = '>';
        rightBtn.disabled = index >= columns.length - 1;
        rightBtn.addEventListener('click', () => adminMoveCookiePoolColumn(column.key, 1));
        moves.appendChild(leftBtn);
        moves.appendChild(rightBtn);
        th.appendChild(title);
        th.appendChild(moves);
        row.appendChild(th);
    });
    head.appendChild(row);
    const filterRow = document.createElement('tr');
    filterRow.className = 'admin-pool-filter-row';
    columns.forEach((column) => {
        const th = document.createElement('th');
        th.className = `admin-pool-col-${column.group || 'system'}`;
        if (column.key !== 'actions') {
            const input = document.createElement('input');
            input.className = 'text-input admin-pool-filter-input';
            input.type = 'search';
            input.placeholder = 'Loc...';
            input.value = String(adminCookiePoolColumnFilters[column.key] || '');
            input.addEventListener('input', () => {
                adminCookiePoolColumnFilters[column.key] = input.value;
                renderAdminCookiePoolBody(columns);
                updateAdminCookiePoolHiddenFilterWarning();
            });
...
function renderAdminCookiePoolBody(columns = getAdminPoolColumns()) {
    const body = el('adminCookiePoolTableBody');
    if (!body) return;
    body.innerHTML = '';
    const items = getFilteredCookiePoolItems();
    if (items.length === 0) {
        const row = document.createElement('tr');
        appendPoolCell(row, 'Khong co cookie nao trong filter hien tai.');
        row.firstChild.colSpan = columns.length || 1;
        body.appendChild(row);
        return;
    }
    items.forEach((item) => {
        const row = document.createElement('tr');
        const matchesRules = matchesAdminPoolSelectionRules(item);
        if (matchesRules) {
            row.classList.add('admin-pool-row-match');
            row.title = 'Dang dat bo dieu kien lay cookie';
        }
        columns.forEach((column) => {
            const cell = document.createElement('td');
            cell.className = `admin-pool-col-${column.group || 'system'}`;
            if (column.key === 'actions') {
                const actions = document.createElement('div');
                actions.className = 'admin-pool-row-actions';
                if (matchesRules) {
                    const badge = document.createElement('span');
                    badge.className = 'admin-pool-match-badge';
                    badge.textContent = 'Dat dieu kien';
                ...
                saveBtn.addEventListener('click', () => adminSaveCookiePoolItem(item.id));
                const checkBtn = document.createElement('button');
                checkBtn.className = 'btn btn-ghost';
                checkBtn.type = 'button';
                checkBtn.textContent = 'Check';
                checkBtn.addEventListener('click', () => adminCheckCookiePoolItems([item.id]));
                const deleteBtn = document.createElement('button');
                deleteBtn.className = 'btn btn-ghost';
                deleteBtn.type = 'button';
                deleteBtn.textContent = 'Xoa';
                deleteBtn.addEventListener('click', () => adminDeleteCookiePoolItem(item.id));
                actions.appendChild(saveBtn);
                actions.appendChild(checkBtn);
                actions.appendChild(deleteBtn);
                cell.appendChild(actions);
            } else if (column.editable) {
                const input = createAdminPoolInput(item.id, column.key, getAdminPoolCellValue(item, column), column.key === 'cookie' ? 'admin-pool-cookie-input' : '');
                cell.appendChild(input);
            } else {
                cell.textContent = String(getAdminPoolCellValue(item, column) || '').trim();
            }
            row.appendChild(cell);
        });
        body.appendChild(row);
...
function renderAdminCookiePoolTable() {
    const columns = getAdminPoolColumns();
    renderAdminCookiePoolHeader(columns);
    renderAdminCookiePoolBody(columns);
    renderAdminCookiePoolVisibilityPanel();
    updateAdminCookiePoolHiddenFilterWarning();
}

function clearAdminCookiePoolFilters() {
    adminCookiePoolColumnFilters = {};
    const filter = el('adminCookiePoolFilter');
    if (filter) filter.value = 'all';
    renderAdminCookiePoolTable();
}

function renderAdminCookiePoolVisibilityPanel() {
    const panel = el('adminCookiePoolVisibilityPanel');
    if (!panel) return;
    const config = getAdminCookiePoolConfig();
    const hidden = new Set(config.hiddenColumns || []);
    panel.innerHTML = '';
    ...
    columns.forEach((column) => {
        ...
        label.className = 'admin-pool-visibility-option';
        const checkbox = document.createElement('input');
        checkbox.type = 'checkbox';
        checkbox.checked = column.key === 'actions' || !hidden.has(column.key);
        checkbox.disabled = column.key === 'actions';
        checkbox.addEventListener('change', async () => {
            const nextHidden = new Set(getAdminCookiePoolConfig().hiddenColumns || []);
            if (checkbox.checked) nextHidden.delete(column.key);
            else nextHidden.add(column.key);
            await saveAdminCookiePoolConfig({
                ...getAdminCookiePoolConfig(),
    ...
    showAll.type = 'button';
    showAll.textContent = 'Hien tat ca cot';
...
function updateAdminCookiePoolHiddenFilterWarning() {
    const node = el('adminCookiePoolHiddenFilterWarning');
    if (!node) return;
    const config = getAdminCookiePoolConfig();
    const hidden = new Set(config.hiddenColumns || []);
    const hiddenFiltered = Object.entries(adminCookiePoolColumnFilters)
        .filter(([key, value]) => hidden.has(key) && String(value || '').trim())
        .map(([key]) => key);
    node.textContent = hiddenFiltered.length > 0
        ? 'Dang loc tren cot dang an. Mo An/hien cot de xem hoac xoa loc.'
        : '';
    node.classList.toggle('hidden', hiddenFiltered.length === 0);
}

const ADMIN_POOL_RULE_OPERATORS = [
    ['equals', '='],
    ['not_equals', 'khác'],
    ['contains', 'chứa'],
...
function getAdminPoolRuleFields() {
    const systemFields = ADMIN_POOL_SYSTEM_COLUMNS
        .filter((column) => !['cookie', 'actions'].includes(column.key))
        .map((column) => ({ value: column.key, label: column.label }));
    const customFields = getAdminCookiePoolConfig().customColumns.map((column) => ({
...
function makeAdminPoolSelect(options = [], value = '', className = '') {
    const select = document.createElement('select');
    select.className = `text-input admin-pool-rule-control${className ? ` ${className}` : ''}`;
...
function renderAdminCookiePoolRules() {
    const container = el('adminCookiePoolRules');
    if (!container) return;
    container.innerHTML = '';
    const config = getAdminCookiePoolConfig();
    const rules = config.selectionRules || { groups: [] };
    const title = document.createElement('div');
    title.className = 'admin-pool-rules-title';
    title.textContent = 'Dieu kien lay cookie: nhom AND, dong trong nhom OR';
    container.appendChild(title);

    const groups = Array.isArray(rules.groups) ? rules.groups : [];
    if (groups.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'admin-pool-rules-empty';
        empty.textContent = 'Chua co dieu kien. Mac dinh chi lay cookie PASS.';
        container.appendChild(empty);
    }

    groups.forEach((group, groupIndex) => {
        const groupBox = document.createElement('div');
        groupBox.className = 'admin-pool-rule-group';
        const groupHeader = document.createElement('div');
        groupHeader.className = 'admin-pool-rule-group-header';
        const groupLabel = document.createElement('strong');
        groupLabel.textContent = `Nhom ${groupIndex + 1} (OR)`;
        const removeGroup = document.createElement('button');
        removeGroup.className = 'admin-pool-mini-btn';
        removeGroup.type = 'button';
        removeGroup.textContent = 'Xoa nhom';
        removeGroup.addEventListener('click', () => {
            config.selectionRules.groups.splice(groupIndex, 1);
            renderAdminCookiePoolRules();
        });
        groupHeader.appendChild(groupLabel);
        groupHeader.appendChild(removeGroup);
        groupBox.appendChild(groupHeader);

        (Array.isArray(group.conditions) ? group.conditions : []).forEach((condition, conditionIndex) => {
            const line = document.createElement('div');
            line.className = 'admin-pool-rule-line';
            const field = makeAdminPoolSelect(getAdminPoolRuleFields().map((item) => [item.value, item.label]), condition.field);
            const operator = makeAdminPoolSelect(ADMIN_POOL_RULE_OPERATORS, condition.operator || 'equals');
            const value = document.createElement('input');
            value.className = 'text-input admin-pool-rule-control';
            value.type = 'text';
            value.value = String(condition.value || '');
            value.placeholder = 'Gia tri';
            const remove = document.createElement('button');
            remove.className = 'admin-pool-mini-btn';
            remove.type = 'button';
            remove.textContent = 'Xoa';
            field.addEventListener('change', () => { condition.field = field.value; });
            operator.addEventListener('change', () => { condition.operator = operator.value; });
        ...
        addCondition.className = 'btn btn-ghost';
        addCondition.type = 'button';
        addCondition.textContent = 'Them dieu kien OR';
        addCondition.addEventListener('click', () => {
            group.conditions = Array.isArray(group.conditions) ? group.conditions : [];
            group.conditions.push({ field: 'status', operator: 'equals', value: 'pass' });
            renderAdminCookiePoolRules();
        });
        groupBox.appendChild(addCondition);
        container.appendChild(groupBox);
    });

    const actions = document.createElement('div');
    actions.className = 'admin-pool-rules-actions';
    const addGroup = document.createElement('button');
    addGroup.className = 'btn btn-ghost';
    addGroup.type = 'button';
    addGroup.textContent = 'Them nhom AND';
    addGroup.addEventListener('click', () => {
        config.selectionRules.groups.push({
    ...
    save.type = 'button';
    save.textContent = 'Luu dieu kien';
    save.addEventListener('click', async () => {
        setAdminCookiePoolState('Dang luu dieu kien...', 'loading');
        try {
            await saveAdminCookiePoolConfig(config);
            renderAdminCookiePoolRules();
            renderAdminCookiePoolTable();
            setAdminCookiePoolState('Da luu dieu kien lay cookie.', 'success');
        } catch (error) {
            setAdminCookiePoolState(error.message || 'Luu dieu kien that bai.', 'error');
        }
    });
...
async function loadAdminCookiePool(options = {}) {
    if (!adminAuthenticated) return;
    if (adminCookiePoolLoading) return;
    adminCookiePoolLoading = true;
    const silent = !!(options && options.silent);
    if (!silent) setAdminCookiePoolState('Dang tai kho cookie noi bo...', 'loading');
    try {
        const [data, configData] = await Promise.all([
            apiRequest('/api/getlink-admin/cookie-pool', 'GET'),
            apiRequest('/api/getlink-admin/cookie-pool/config', 'GET')
        ]);
        adminCookiePoolItems = Array.isArray(data.items) ? data.items : [];
        adminCookiePoolConfig = normalizeAdminCookiePoolConfig(configData && configData.config ? configData.config : {});
        adminCookiePoolLoaded = true;
        renderAdminCookiePoolRules();
        renderAdminCookiePoolTable();
        setAdminCookiePoolState(`Da tai ${adminCookiePoolItems.length} cookie trong kho noi bo.`, 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Khong tai duoc kho cookie noi bo.', 'error');
    } finally {
        adminCookiePoolLoading = false;
    }
}

async function adminImportCookiePoolPaste() {
    if (!adminAuthenticated) {
        setAdminCookiePoolState('Ban chua dang nhap admin.', 'warning');
        return;
    }
    const rows = parseCookiePoolPaste(el('adminCookiePoolPasteInput')?.value || '');
    if (rows.length === 0) {
        setAdminCookiePoolState('Chua co cookie de import.', 'warning');
        return;
    }
    const btn = el('adminCookiePoolImportBtn');
    setButtonBusy(btn, true, 'Dang import...');
    setAdminCookiePoolState(`Dang import ${rows.length} dong...`, 'loading');
    try {
        const data = await apiRequest('/api/getlink-admin/cookie-pool/import', 'POST', { rows });
        if (data && data.config) adminCookiePoolConfig = normalizeAdminCookiePoolConfig(data.config);
        if (el('adminCookiePoolPasteInput')) el('adminCookiePoolPasteInput').value = '';
        await loadAdminCookiePool({ silent: true });
        setAdminCookiePoolState(`Da import ${data.imported || 0} cookie. Trung/merge: ${data.duplicates || 0}.`, 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Import cookie pool that bai.', 'error');
    } finally {
        setButtonBusy(btn, false);
    }
}
...
function readFileAsDataUrl(file) {
    return new Promise((resolve, reject) => {
        ...
        reader.onerror = () => reject(new Error('Khong doc duoc tep Excel.'));
        reader.readAsDataURL(file);
    });
}

async function adminImportCookiePoolFile() {
    if (!adminAuthenticated) {
        setAdminCookiePoolState('Ban chua dang nhap admin.', 'warning');
        return;
    }
    const input = el('adminCookiePoolFileInput');
    const file = input && input.files ? input.files[0] : null;
    if (!file) {
        setAdminCookiePoolState('Chua chon tep Excel.', 'warning');
        return;
    }
    if (!/\.(xlsx|xls)$/i.test(String(file.name || ''))) {
        setAdminCookiePoolState('Chi ho tro tep Excel .xlsx hoac .xls.', 'warning');
        input.value = '';
        return;
    }
    if (Number(file.size || 0) > 5 * 1024 * 1024) {
        setAdminCookiePoolState('Tep Excel khong duoc vuot qua 5 MB.', 'warning');
        input.value = '';
        return;
    }
    const btn = el('adminCookiePoolImportFileBtn');
    setButtonBusy(btn, true, 'Dang doc Excel...');
    setAdminCookiePoolState(`Dang nhap cookie tu ${file.name}...`, 'loading');
    try {
        const dataUrl = await readFileAsDataUrl(file);
        const separator = dataUrl.indexOf(',');
        const contentBase64 = separator >= 0 ? dataUrl.slice(separator + 1) : dataUrl;
        const data = await apiRequest('/api/getlink-admin/cookie-pool/import-file', 'POST', {
            filename: file.name,
            contentBase64
        });
        if (data && data.config) adminCookiePoolConfig = normalizeAdminCookiePoolConfig(data.config);
        await loadAdminCookiePool({ silent: true });
        setAdminCookiePoolState(
            `Da import ${data.imported || 0} cookie tu Excel. Trung: ${data.duplicates || 0}. O trong: ${data.skippedEmpty || 0}.`,
            'success'
        );
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Import Excel that bai.', 'error');
    } finally {
        input.value = '';
        setButtonBusy(btn, false);
    }
...
async function adminCheckCookiePoolItems(ids = []) {
    if (!adminAuthenticated) {
        setAdminCookiePoolState('Ban chua dang nhap admin.', 'warning');
        return;
    }
    const all = !Array.isArray(ids) || ids.length === 0;
    const btn = all ? el('adminCookiePoolCheckAllBtn') : null;
    setButtonBusy(btn, true, 'Dang check...');
    setAdminCookiePoolState(all ? 'Dang check toan bo kho cookie...' : 'Dang check cookie da chon...', 'loading');
    try {
        const data = await apiRequest('/api/getlink-admin/cookie-pool/check', 'POST', { ids: Array.isArray(ids) ? ids : [] });
        await loadAdminCookiePool({ silent: true });
        setAdminCookiePoolState(`Da check ${data.checked || 0} cookie.`, 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Check kho cookie that bai.', 'error');
    } finally {
        setButtonBusy(btn, false);
    }
}

async function adminSaveCookiePoolItem(id = '') {
    const itemId = String(id || '').trim();
    const item = adminCookiePoolItems.find((entry) => entry.id === itemId);
    if (!item) return;
    const next = {
        ...item,
        customFields: {
    ...
    document.querySelectorAll(`[data-pool-item-id="${CSS.escape(itemId)}"]`).forEach((input) => {
        const key = String(input && input.dataset ? input.dataset.poolField : '').trim();
        if (!key) return;
        if (key.startsWith('custom:')) {
            next.customFields[key.slice('custom:'.length)] = String(input.value || '').trim();
            return;
        }
        next[key] = String(input.value || '').trim();
    });
    setAdminCookiePoolState('Dang luu cookie...', 'loading');
    try {
        const data = await apiRequest(`/api/getlink-admin/cookie-pool/${encodeURIComponent(itemId)}`, 'PUT', {
            ...next
        });
        const savedItem = data.item || next;
        adminCookiePoolItems = adminCookiePoolItems.filter((entry) => entry.id !== itemId && entry.id !== savedItem.id);
        adminCookiePoolItems.unshift(savedItem);
        renderAdminCookiePoolTable();
        setAdminCookiePoolState('Da luu cookie.', 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Luu cookie that bai.', 'error');
    }
}

async function saveAdminCookiePoolConfig(config = {}) {
    const current = getAdminCookiePoolConfig();
    const nextConfig = normalizeAdminCookiePoolConfig({
        ...current,
        ...config,
        customColumns: config.customColumns !== undefined ? config.customColumns : current.customColumns,
        columnOrder: config.columnOrder !== undefined ? config.columnOrder : current.columnOrder,
        hiddenColumns: config.hiddenColumns !== undefined ? config.hiddenColumns : current.hiddenColumns,
        selectionRules: config.selectionRules !== undefined ? config.selectionRules : current.selectionRules
    });
    const data = await apiRequest('/api/getlink-admin/cookie-pool/config', 'PUT', { config: nextConfig });
    adminCookiePoolConfig = normalizeAdminCookiePoolConfig(data && data.config ? data.config : nextConfig);
    renderAdminCookiePoolRules();
    renderAdminCookiePoolVisibilityPanel();
    renderAdminCookiePoolTable();
    return adminCookiePoolConfig;
...
async function adminAddCookiePoolColumn() {
    const label = String(window.prompt('Ten cot moi:', '') || '').trim();
    if (!label) return;
    const config = getAdminCookiePoolConfig();
    const key = normalizeAdminPoolCustomKey(label);
    if (config.customColumns.some((column) => column.key === key)) {
        setAdminCookiePoolState('Cot nay da ton tai.', 'warning');
        return;
    }
    const customColumns = [...config.customColumns, { key, label }];
    const columnOrder = [...config.columnOrder];
    const actionIndex = columnOrder.indexOf('actions');
    const customKey = `custom:${key}`;
    if (actionIndex >= 0) columnOrder.splice(actionIndex, 0, customKey);
    else columnOrder.push(customKey);
    setAdminCookiePoolState('Dang them cot...', 'loading');
    try {
        await saveAdminCookiePoolConfig({ customColumns, columnOrder });
        setAdminCookiePoolState('Da them cot.', 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Them cot that bai.', 'error');
    }
}

async function adminRenameCookiePoolColumn(customKey = '') {
    const key = String(customKey || '').trim();
    const config = getAdminCookiePoolConfig();
    const column = config.customColumns.find((item) => item.key === key);
    if (!column) return;
    const label = String(window.prompt('Doi ten cot:', column.label) || '').trim();
    if (!label) return;
    setAdminCookiePoolState('Dang doi ten cot...', 'loading');
    try {
        await saveAdminCookiePoolConfig({
            ...config,
            customColumns: config.customColumns.map((item) => item.key === key ? { ...item, label } : item)
        });
        setAdminCookiePoolState('Da doi ten cot.', 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Doi ten cot that bai.', 'error');
    }
}

async function adminDeleteCookiePoolColumn(customKey = '') {
    const key = String(customKey || '').trim();
    if (!key || !window.confirm('Xoa cot tuy bien nay khoi bang?')) return;
    const config = getAdminCookiePoolConfig();
    setAdminCookiePoolState('Dang xoa cot...', 'loading');
    try {
        await saveAdminCookiePoolConfig({
            customColumns: config.customColumns.filter((column) => column.key !== key),
            columnOrder: config.columnOrder.filter((columnKey) => columnKey !== `custom:${key}`)
        });
        setAdminCookiePoolState('Da xoa cot.', 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Xoa cot that bai.', 'error');
    }
}

async function adminMoveCookiePoolColumn(columnKey = '', direction = 0) {
    const key = String(columnKey || '').trim();
    const offset = Number(direction || 0) < 0 ? -1 : 1;
    const config = getAdminCookiePoolConfig();
    const columnOrder = [...config.columnOrder];
    const index = columnOrder.indexOf(key);
    const visibleKeys = getAdminPoolColumns().map((column) => column.key);
    const visibleIndex = visibleKeys.indexOf(key);
    const targetKey = visibleIndex >= 0 ? visibleKeys[visibleIndex + offset] : '';
    const targetIndex = columnOrder.indexOf(targetKey);
    if (index < 0 || visibleIndex < 0 || targetIndex < 0) return;
    const [moved] = columnOrder.splice(index, 1);
    const adjustedTargetIndex = columnOrder.indexOf(targetKey);
    const insertIndex = offset < 0 ? adjustedTargetIndex : adjustedTargetIndex + 1;
    columnOrder.splice(insertIndex, 0, moved);
    setAdminCookiePoolState('Dang luu thu tu cot...', 'loading');
    try {
        await saveAdminCookiePoolConfig({ ...config, columnOrder });
        setAdminCookiePoolState('Da luu thu tu cot.', 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Di chuyen cot that bai.', 'error');
    }
}

async function adminDeleteCookiePoolItem(id = '') {
    const itemId = String(id || '').trim();
    if (!itemId) return;
    setAdminCookiePoolState('Dang xoa cookie...', 'loading');
    try {
        await apiRequest(`/api/getlink-admin/cookie-pool/${encodeURIComponent(itemId)}`, 'DELETE');
        adminCookiePoolItems = adminCookiePoolItems.filter((item) => item.id !== itemId);
        renderAdminCookiePoolTable();
        setAdminCookiePoolState('Da xoa cookie khoi kho noi bo.', 'success');
    } catch (error) {
        setAdminCookiePoolState(error.message || 'Xoa cookie that bai.', 'error');
    }
}
...
function formatAutoFixTimings(timings = null) {
    ...
    pushPart('Source', source.sheetFetchMs);
...
function renderAdminWorkspace() {
    ...
    if (adminAuthenticated) {
        ...
        if (!adminCookiePoolLoaded && !adminCookiePoolLoading) {
            loadAdminCookiePool({ silent: true });
        }
    }
    if (!adminAuthenticated) {
...
function openAdminModal() {
    ...
    if (adminAuthenticated) {
        ...
        loadAdminCookiePool({ silent: !adminCookiePoolLoaded });
    }
}
...
function bindEvents() {
    ...
    const adminCookiePoolImportBtn = el('adminCookiePoolImportBtn');
    if (adminCookiePoolImportBtn) adminCookiePoolImportBtn.addEventListener('click', adminImportCookiePoolPaste);

    const adminCookiePoolImportFileBtn = el('adminCookiePoolImportFileBtn');
    const adminCookiePoolFileInput = el('adminCookiePoolFileInput');
    if (adminCookiePoolImportFileBtn && adminCookiePoolFileInput) {
        adminCookiePoolImportFileBtn.addEventListener('click', () => adminCookiePoolFileInput.click());
        adminCookiePoolFileInput.addEventListener('change', adminImportCookiePoolFile);
    }

    const adminCookiePoolVisibilityBtn = el('adminCookiePoolVisibilityBtn');
    const adminCookiePoolVisibilityPanel = el('adminCookiePoolVisibilityPanel');
    if (adminCookiePoolVisibilityBtn && adminCookiePoolVisibilityPanel) {
        adminCookiePoolVisibilityBtn.addEventListener('click', () => {
            adminCookiePoolVisibilityPanel.classList.toggle('hidden');
        });
    }

    const adminCookiePoolAddColumnBtn = el('adminCookiePoolAddColumnBtn');
    if (adminCookiePoolAddColumnBtn) adminCookiePoolAddColumnBtn.addEventListener('click', adminAddCookiePoolColumn);

    const adminCookiePoolCheckAllBtn = el('adminCookiePoolCheckAllBtn');
    if (adminCookiePoolCheckAllBtn) adminCookiePoolCheckAllBtn.addEventListener('click', () => adminCheckCookiePoolItems([]));

    const adminCookiePoolReloadBtn = el('adminCookiePoolReloadBtn');
    if (adminCookiePoolReloadBtn) adminCookiePoolReloadBtn.addEventListener('click', () => loadAdminCookiePool({ silent: false }));

    const adminCookiePoolFilter = el('adminCookiePoolFilter');
    if (adminCookiePoolFilter) adminCookiePoolFilter.addEventListener('change', renderAdminCookiePoolTable);

    const adminCookiePoolClearFiltersBtn = el('adminCookiePoolClearFiltersBtn');
    if (adminCookiePoolClearFiltersBtn) adminCookiePoolClearFiltersBtn.addEventListener('click', clearAdminCookiePoolFilters);
